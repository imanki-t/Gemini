...
apiKeys.forEach((_, idx) => {
  keyUsageStats.set(idx, { requests: 0, lastUsed: null, errors: 0 });
});

// Initialize the first client with CORRECT SDK
let currentClient = new GoogleGenAI({ apiKey: apiKeys[0] });

// Log key switch function
function logKeySwitch(oldIdx, newIdx, reason) {
  ...
  console.log(`   Total Requests on Old Key: ${keyUsageStats.get(oldIdx).requests}`);
  console.log(`${'='.repeat(60)}\n`);
}

// 2. Proxy to handle rotation seamlessly with enhanced logging
export const genAI = new Proxy({}, {
  get(target, prop) {
    // Intercept access to main API features
    if (['models', 'chats', 'files', 'caches'].includes(prop)) {
      requestCount++;
      
      // Update current key stats
      ...
      if (requestCount >= ROTATION_THRESHOLD && apiKeys.length > 1) {
        ...
        logKeySwitch(oldIdx, currentKeyIdx, `Request threshold reached (${ROTATION_THRESHOLD} requests)`);
      }
    }
...
// Function to manually switch keys (useful for error handling)
export function forceKeySwitch(reason = 'Manual switch') {
  if (apiKeys.length <= 1) {
    console.log('⚠️ Cannot switch keys - only one key available');
    return false;
  }
...
// Function to get current API key stats
