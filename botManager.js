...
// Export a wrapper that provides the correct API structure
export const genAI = {
  models: {
    generateContent: (request) => withRetry(() => {
      return currentClient.models.generateContent(request);
    }),
    generateContentStream: (request) => withRetry(async () => {
      // Return the async iterator directly
      return currentClient.models.generateContentStream(request);
    }),
    embedContent: (request) => withRetry(() => {
      return currentClient.models.embedContent(request);
    })
  },
  files: {
    upload: (options) => withRetry(() => {
      return currentClient.files.upload(options);
    }),
    get: (options) => withRetry(() => {
      return currentClient.files.get(options);
    })
  }
};
