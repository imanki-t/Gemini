...
try {
  ...
  // Determine if continuous reply is active
  const isContinuous = (guildId && (channelContinuousReply || continuousReply)) || isDM;
  ...
  if (shouldRespond) {
    if (activeRequests.has(userId)) {
      ...
      // Send as a direct message, not a reply
      ...
      await handleTextMessage(message, isContinuous); // Pass isContinuous flag
    }
  }
} catch (error) {
  ...
  while (attempts > 0 && !stopGeneration) {
    try {
      ...
      if (!isLargeResponse) {
        ...
        // For interactions, we don't need to worry about mentions, so isContinuous is effectively true
        await sendAsTextFile(finalResponse, interaction, botMessage.id, true);
      }
...
const supportedTextExtensions = ['.html', '.js', '.css', '.json', '.xml', '.csv', '.py', '.java', '.sql', '.log', '.md', '.txt', '.docx', '.pptx'];
...
} else if (supportedTextExtensions.includes(fileExtension)) {
  try {
...
try {
  ...
  if (isUpdate) {
    ...
    // Add 5-minute expiry timer
    setTimeout(async () => {
      try {
        const expiredEmbed = new EmbedBuilder()
          .setColor(0xFFAA00)
          .setTitle('⚙️ Settings Expired')
          .setDescription('This settings panel has expired. Please run `/settings` again.');
        ...
        // Ignore errors (e.g., if user dismissed the message)
        // console.error("Error expiring settings:", error.message);
      }
...
try {
  ...
  const components = [
    ...
    new ActionRowBuilder().addComponents(...buttons2) // Always add buttons2 row
  ];
...
try {
  ...
  const buttons1 = [
    ...
    .setStyle(ButtonStyle.Danger)
    .setDisabled(!hasPersonality),
    new ButtonBuilder() // FIX: Was ButtonButton
  ...
  const components = [
    new ActionRowBuilder().addComponents(continuousReplySelect),
    new ActionRowSBuilder().addComponents(chatHistorySelect),
    new ActionRowBuilder().addComponents(...buttons1),
...
try {
  ...
  const historyTitle = isDM 
      ? '📥 **Your DM Conversation History**'
      : '📥 **Your Personal Conversation History**';
...
try {
  ...
  const file = new AttachmentBuilder(tempFileName, {
    name: `server_conversation_${guildName}.txt`
  });

  const historyTitle = `📥 **Conversation History for ${guildName} Server**`;
...
try {
  ...
  const contentPreview = textContent.length > 1950 ? textContent.slice(0, 1950) + '...' : textContent;

  const embed = new EmbedBuilder()
    .setColor(0x00FF00)
    .setTitle('💾 Message Saved')
    .setDescription('The message content and file are below (or sent to your DMs).');
  ...
  if (interaction.channel.type === ChannelType.DM) {
    await interaction.reply({
      ...
      embeds: [embed],
      files: [attachment],
      flags: MessageFlags.Ephemeral // Keep it ephemeral so only user sees it
    });
  } else {
    try {
      await interaction.user.send({
        content: textContent, // Send full content to DM
        embeds: [embed.setDescription('Here is the message content and file you saved:')],
        files: [attachment]
      });
      const dmSentEmbed = new EmbedBuilder()
        .setColor(0x00FF00)
        .setTitle('✅ Sent to DMs')
        .setDescription('The message content and file have been sent to your DMs!');
      await interaction.reply({
        embeds: [dmSentEmbed],
        flags: MessageFlags.Ephemeral
      });
      ...
      const failDMEmbed = new EmbedBuilder()
        .setColor(0xFF5555)
        .setTitle('❌ DM Failed')
        .setDescription('Could not send to DMs. Here is the message content and file:');
      await interaction.reply({
        content: contentPreview,
        embeds: [failDMEmbed],
        files: [attachment],
  ...
  const errorEmbed = new EmbedBuilder()
      .setColor(0xFF0000)
      .setTitle('❌ Error')
      .setDescription('An error occurred while trying to save the message.');
  try {
      ...
      console.error("Error sending download error reply:", e);
  }
}
}

// This function is no longer used by downloadMessage, but kept in case other parts rely on it.
...
async function handleTextMessage(message, isContinuous) {
...
try {
  ...
  botMessage = isContinuous
    ? await message.channel.send({ content: 'Gemini is thinking...' })
    : await message.reply({ content: 'Gemini is thinking...' });
...
await handleModelResponse(botMessage, chat, parts, message, typingInterval, historyId, effectiveSettings, isContinuous);
}
...
try {
  ...
  if(actionRow.components.length < 5) {
      ...
      return await botMessage.edit({ components: [actionRow, newRow] });
  }
...
async function handleModelResponse(initialBotMessage, chat, parts, originalMessage, typingInterval, historyId, effectiveSettings, isContinuous) {
...
const updateMessage = () => {
  ...
  if (tempResponse.trim() === "") {} else if (responseFormat === 'Embedded') {
    updateEmbed(botMessage, tempResponse, originalMessage, groundingMetadata, urlContextMetadata, effectiveSettings, isContinuous);
  } else {
    const mention = isContinuous ? '' : `<@${originalMessage.author.id}> `;
    botMessage.edit({
...
while (attempts > 0 && !stopGeneration) {
  try {
    ...
    if (!isLargeResponse && responseFormat === 'Embedded') {
      updateEmbed(botMessage, finalResponse, originalMessage, groundingMetadata, urlContextMetadata, effectiveSettings, isContinuous);
    } else if (!isLargeResponse) {
      const mention = isContinuous ? '' : `<@${originalMessage.author.id}> `;
      botMessage.edit({
        content: mention + finalResponse.slice(0, 2000),
        embeds: []
      }).catch(() => {});
    }
    ...
    if (isLargeResponse) {
      await sendAsTextFile(finalResponse, originalMessage, botMessage.id, isContinuous);
      if (showActionButtons) {
    ...
    if (attempts === 0 || stopGeneration) {
      if (!stopGeneration) {
        ...
        const errorMsg = await originalMessage.channel.send({
          content: isContinuous ? '' : `<@${originalMessage.author.id}>`,
          embeds: [embed]
        });
...
function updateEmbed(botMessage, finalResponse, message, groundingMetadata = null, urlContextMetadata = null, effectiveSettings, isContinuous) {
try {
...
async function sendAsTextFile(text, messageOrInteraction, orgId, isContinuous = false) {
try {
  ...
  const mention = (isInteraction || !isContinuous) ? `<@${userId}>, ` : '';
