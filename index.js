...
try {
 ...
 if (channel) {
   channel.send(`Glad to join ${guild.name} !!`);
 }
} catch (error) {
 console.error(`Could not send welcome message to ${guild.name}:`, error);
}
});
...
if (shouldUploadToAPI) {
 ...
 try {
   ...
   const needsProcessingCheck =
     (contentType.startsWith('video/') || videoExtensions.includes(fileExtension)) ||
   ...
   if (needsProcessingCheck) {
     ...
     let attempts = 0;
     const isVideo = contentType.startsWith('video/') || videoExtensions.includes(fileExtension);
     const maxAttempts = isVideo ? 60 : 30;
     const delayTime = isVideo ? 10000 : 2000;
...
while (attempts > 0 && !stopGeneration) {
 try {
   ...
   } else if (!isLargeResponse && responseFormat !== 'Embedded') {
     try {
       await botMessage.delete().catch(() => {});
     } catch (e) {
       console.error("Error deleting 'thinking' message:", e);
     }
