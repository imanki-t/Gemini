...
// ** FIX 2: Continuous Reply State Update **
// If continuous reply is active and in a guild channel, mark the user as 'active' 
// in the channel's context to allow the next non-mentioned message to be processed.
if (effectiveConfig.continuousReply && guildId) { 
 if (!state.activeUsersInChannels[channelId]) {
   state.activeUsersInChannels[channelId] = {};
 }
 // Set user as active. True/timestamp is fine for the current logic check.
...
try { // ** FIX 1: Ensure activeRequests is cleared with a try/finally block **
 const updateMessage = () => {
...
app.get('/', (req, res) => {
res.status(200).send(`Bot is running and web server is listening on port ${PORT}.`);
});
