   ...
   if (guildId) {
     ...
     if (message.attachments.size > 5) {
       const embed = new EmbedBuilder()
         ...
         .setDescription('Please provide a maximum of 5 attachments at a time in a server.');
       await message.reply({
         embeds: [embed]
       });
       return;
...
async function handleSearchCommand(interaction) {
 try {
   ...
   const hasNonTextMedia = attachment && (attachment.contentType.startsWith('video/') || attachment.contentType.startsWith('audio/') || attachment.contentType.startsWith('image/'));
...
async function handleSelectMenuInteraction(interaction) {
 ...
 try {
   ...
   const menuHandlers = {
     ...
     'user_response_format': handleUserResponseFormatSelect,
     'server_response_format': handleServerResponseFormatSelect,
     'user_action_buttons': handleUserActionButtonsSelect,
     'server_action_buttons': handleServerActionButtonsSelect,
     'user_continuous_reply': handleUserContinuousReplySelect,
     'server_continuous_reply': handleServerContinuousReplySelect,
     'server_override': handleServerOverrideSelect,
     'server_chat_history': handleServerChatHistorySelect,
   ...
   if (interaction.replied || interaction.deferred) {
     await interaction.followUp({
       content: 'An error occurred while handling this menu.',
       flags: MessageFlags.Ephemeral
     }).catch(console.error);
   } else {
     await interaction.reply({
...
async function handleServerModelSelect(interaction) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
   });
 }
 const selectedModel = interaction.values[0];
...
async function handleUserResponseFormatSelect(interaction) {
...
async function handleServerResponseFormatSelect(interaction) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
...
async function handleUserActionButtonsSelect(interaction) {
...
async function handleServerActionButtonsSelect(interaction) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
...
async function handleUserContinuousReplySelect(interaction) {
 const selectedValue = interaction.values[0];
 const userId = interaction.user.id;
 if (!state.userSettings[userId]) {
   state.userSettings[userId] = {};
...
async function handleServerContinuousReplySelect(interaction) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
...
async function handleServerChatHistorySelect(interaction) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
...
async function handleModalSubmit(interaction) {
 try {
   ...
   const modalHandlers = {
     ...
     'server_embed_color_modal': handleServerEmbedColorModal,
   };

   const handler = modalHandlers[interaction.customId];
   if (handler) {
     await handler(interaction, userId, guildId);
   } else {
     console.warn(`Unknown modal submit: ${interaction.customId}`);
   }
 } catch (error) {
   console.error('Error in handleModalSubmit:', error);
   if (interaction.replied || interaction.deferred) {
     await interaction.followUp({
       content: 'An error occurred while submitting this form.',
       flags: MessageFlags.Ephemeral
     }).catch(console.error);
   } else {
     await interaction.reply({
...
async function handleUserPersonalityModal(interaction, userId, guildId) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to save personality.',
     flags: MessageFlags.Ephemeral
   });
 }
}

async function handleServerPersonalityModal(interaction, userId, guildId) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
 ...
 try {
   ...
   await interaction.reply({
     content: 'Failed to save server personality.',
     flags: MessageFlags.Ephemeral
   });
 }
}

async function handleUserEmbedColorModal(interaction, userId, guildId) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to save embed color.',
     flags: MessageFlags.Ephemeral
   });
 }
}

async function handleServerEmbedColorModal(interaction, userId, guildId) {
 if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
   return interaction.reply({
     embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('🚫 Permission Denied').setDescription('You need "Manage Server" permission to change server settings.')],
     flags: MessageFlags.Ephemeral
 ...
 try {
   ...
   await interaction.reply({
     content: 'Failed to save server embed color.',
     flags: MessageFlags.Ephemeral
   });
 }
}
...
async function handleShowUserSettings(interaction, isUpdate = true) {
 try {
   ...
   console.error('Error in handleShowUserSettings:', error);
 }
}
...
async function clearUserMemory(interaction) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to clear user memory.',
     flags: MessageFlags.Ephemeral
   });
 }
}
...
async function clearServerMemory(interaction) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to clear server memory.',
     flags: MessageFlags.Ephemeral
   });
 }
}
...
async function showUserPersonalityModal(interaction) {
 try {
   ...
   console.error('Error showing user personality modal:', error);
 }
}
...
async function showServerPersonalityModal(interaction) {
 try {
   ...
   console.error('Error showing server personality modal:', error);
 }
}
...
async function removeUserPersonality(interaction) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to remove user personality.',
     flags: MessageFlags.Ephemeral
   });
 }
}
...
async function removeServerPersonality(interaction) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to remove server personality.',
     flags: MessageFlags.Ephemeral
   });
 }
}
...
async function showUserEmbedColorModal(interaction) {
 try {
   ...
   console.error('Error showing user embed color modal:', error);
 }
}
...
async function showServerEmbedColorModal(interaction) {
 try {
   ...
   console.error('Error showing server embed color modal:', error);
 }
}
...
async function showChannelManagementMenu(interaction, isUpdate = false) {
 try {
   ...
   console.error('Error showing channel management menu:', error);
 }
}
...
async function toggleContinuousReplyChannel(interaction) {
 try {
   ...
   await interaction.reply({
     content: 'Failed to toggle continuous reply.',
     flags: MessageFlags.Ephemeral
   });
 }
}
...
async function handleDeleteMessageInteraction(interaction, msgId) {
 try {
   ...
   if (userChatHistory && userChatHistory[msgId]) {
     ...
     if (repliedToMessage && repliedToMessage.author.id === userId) {
       canDelete = true;
     }
   } else if (message && message.interaction && message.interaction.user.id === userId) {
     canDelete = true;
   }

   if (canDelete) {
     await interaction.message.delete().catch(err => console.error('Error deleting interaction message:', err));
     if (message) {
       await message.delete().catch(err => console.error('Error deleting original bot message:', err));
     }
   } else {
     const embed = new EmbedBuilder()
       .setColor(0xFF0000)
...
async function downloadMessage(interaction) {
 try {
   ...
   if ((!textContent || textContent.trim() === ' ') && message.embeds.length > 0) {
...
async function stopGeneration(interaction) {
 try {
   ...
   console.error('Error replying to stopGeneration interaction:', error);
 }
}
...
async function handleTextMessage(message) {
 ...
 const hasNonTextMedia = message.attachments.some(att =>
   att.contentType && (att.contentType.startsWith('video/') || att.contentType.startsWith('audio/') || att.contentType.startsWith('image/'))
 );
 ...
 try {
   ...
   if (actionRow.components.length < 5) {
     ...
     return await botMessage.edit({
       components: [actionRow, ...messageComponents.slice(1)]
     });
   } else {
     const newRow = new ActionRowBuilder().addComponents(downloadButton);
     const existingRows = messageComponents.map(c => ActionRowBuilder.from(c));
     return await botMessage.edit({
       components: [...existingRows, newRow].slice(0, 5)
     });
   }
 } catch (error) {
...
async function addDeleteButton(botMessage, msgId) {
 try {
   ...
   for (let i = 0; i < messageComponents.length; i++) {
     if (messageComponents[i].type === ComponentType.ActionRow && messageComponents[i].components.length < 5) {
       targetRowIndex = i;
       break;
     }
   }

   let existingRows = messageComponents.map(c => ActionRowBuilder.from(c));

   if (targetRowIndex !== -1) {
     existingRows[targetRowIndex].addComponents(deleteButton);
     return await botMessage.edit({
     ...
     return await botMessage.edit({
       components: [...existingRows, newRow]
     });
   } else {
     existingRows[existingRows.length - 1].components.pop();
     existingRows[existingRows.length - 1].addComponents(deleteButton);
     return await botMessage.edit({
       components: existingRows
     });
   }
...
async function handleModelResponse(initialBotMessage, chat, parts, originalMessage, typingInterval, historyId, effectiveSettings) {
 ...
 try {
   ...
   console.error("Failed to add stop button:", e.message);
 }

 const filter = (interaction) => interaction.customId === 'stopGenerating' && interaction.user.id === originalMessage.author.id;
 let collector;
 try {
   collector = await botMessage.createMessageComponentCollector({
     filter,
     time: 120000
   });
 ...
 try {
     ...
     try {
       ...
       if (isLargeResponse) {
         ...
         if (showActionButtons) {
           ...
           botMessage.edit({
             components: [botMessage.components[0]]
           }).catch(() => {});
         }
       }
   ...
   console.error("Critical error in handleModelResponse:", e);
   clearInterval(typingInterval);
 } finally {
 ...
 try {
   ...
   if (continuousReply) {
     ...
     botMessage = await channel.send({
       ...
       files: [tempFilePath],
       reply: {
         messageReference: isInteraction ? undefined : messageOrInteraction.id,
