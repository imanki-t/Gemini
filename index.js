...
/**
* Starts an expiry collector on a message to delete it after 5 minutes of inactivity.
* Resets the timer on any component interaction.
* @param {import('discord.js').Interaction} interaction - The interaction to reply to.
* @param {import('discord.js').Message} message - The message to watch for inactivity.
*/
async function startExpiryCollector(interaction, message) {
try {
  ...
  // We just need to reset the timer on any interaction.
  // The main 'interactionCreate' handler will still process the button/menu logic.
  collector.on('collect', () => {
    collector.resetTimer();
  });

  // When the collector times out, delete the reply.
  ...
  console.error('Failed to start expiry collector:', error);
}
}
...
try {
  ...
  startExpiryCollector(interaction, replyMessage);
...
try {
  ...
  const components = [
    ...
    new ActionRowBuilder().addComponents(...buttons2) // Added the second row of buttons
  ];
...
try {
  ...
  const buttons1 = [
    ...
    .setStyle(ButtonStyle.Danger)
    .setDisabled(!hasPersonality),
    new ButtonBuilder() // <-- Fixed typo here (was ButtonButton)
...
// Get settings *before* sending the first message
const userSettings = state.userSettings[userId] || {};
const serverSettings = guildId ? (state.serverSettings[guildId] || {}) : {};
const effectiveSettings = serverSettings.overrideUserSettings ? serverSettings : userSettings;
const channelContinuousReply = state.continuousReplyChannels?.[channelId] || false;
const effectiveContinuous = effectiveSettings.continuousReply || channelContinuousReply;
const sendAsReply = !effectiveContinuous && !isDM;
...
// We already have effectiveSettings from above
let instructions;
if (guildId) {
...
const channelContinuousReply = state.continuousReplyChannels?.[originalMessage.channel.id] || false;
const effectiveContinuous = effectiveSettings.continuousReply || channelContinuousReply;
...
const updateMessage = () => {
  ...
  if (tempResponse.trim() === "") {} else if (responseFormat === 'Embedded') {
    ...
    const mention = (effectiveContinuous || isDM) ? '' : `<@${originalMessage.author.id}> `;
    botMessage.edit({
...
while (attempts > 0 && !stopGeneration) {
  try {
    ...
    if (isLargeResponse) {
      await sendAsTextFile(finalResponse, originalMessage, botMessage.id, effectiveContinuous);
      if (showActionButtons) {
...
try {
  ...
  const isDM = message.channel.type === ChannelType.DM;
  const channelContinuousReply = state.continuousReplyChannels?.[message.channel.id] || false;
  const serverContinuous = effectiveSettings.continuousReply || false;
  const embedColor = effectiveSettings.embedColor || hexColour;
  ...
  const shouldShowAuthor = !isDM && !channelContinuousReply && !serverContinuous;
  if (shouldShowAuthor) {
    embed.setAuthor({
...
try {
  ...
  const mention = (isInteraction || (!continuousReply && !isDM)) ? `<@${userId}>, ` : '';
