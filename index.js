...
if (!state.joinedGuilds) {
 state.joinedGuilds = [];
}

if (!state.joinedGuilds.includes(guild.id)) {
 state.joinedGuilds.push(guild.id);
 ...
 if (channel) {
   try {
     await channel.send(`Glad to join ${guild.name} !!`);
     sent = true;
   } catch (err) {
     console.error(`Failed to send welcome message to ${guild.name}: ${err.message}`);
   }
 }

 if (!sent) {
   console.log(`Joined ${guild.name}, but couldn't find a suitable channel to send the welcome message.`);
 }
}
});
...
while (attempts > 0 && !stopGeneration) {
 try {
   ...
   if (isLargeResponse) {
     finalMessage = await sendAsTextFile(finalResponse, originalMessage, finalMessage.id, continuousReply);
   } else if (responseFormat === 'Embedded') {
     updateEmbed(finalMessage, finalResponse, originalMessage, groundingMetadata, urlContextMetadata, effectiveSettings);
   } else {
     try {
       await finalMessage.delete().catch(() => {});
     } catch (e) {
       console.error("Failed to delete thinking message:", e);
     }

     if (continuousReply) {
       finalMessage = await originalMessage.channel.send({
         content: finalResponse.slice(0, 2000)
       });
     } else {
       finalMessage = await originalMessage.reply({
         content: finalResponse.slice(0, 2000)
       });
     }
   }
   ...
   if (showActionButtons) {
     ...
     if (finalMessage.id === botMessage.id) {
       finalMessage.edit({
         components: []
       }).catch(() => {});
     }
