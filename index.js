...
async function handleButtonInteraction(interaction) {
  ...
  const buttonHandlers = {
    ...
    'manage_allowed_channels': showChannelManagementModal,
...
async function handleModalSubmit(interaction) {
  ...
  if (interaction.customId === 'user_personality_modal') {
  ...
  } else if (interaction.customId === 'channel_management_modal') {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
      const embed = new EmbedBuilder()
        .setColor(0xFF0000)
        .setTitle('🚫 Permission Denied')
    ...
    try {
      ...
      if (channelInput === '' || channelInput.toLowerCase() === 'all' || channelInput.toLowerCase() === 'none') {
        // Clear restrictions - bot works in all channels
      ...
      // Parse channel IDs or mentions
      const channelIds = channelInput
        .split(/[,\s]+/)
      ...
      if (validChannels.length === 0) {
        const embed = new EmbedBuilder()
          .setColor(0xFF5555)
          .setTitle('❌ No Valid Channels')
          .setDescription('None of the provided channel IDs were valid. Please try again.');
        return interaction.reply({
          embeds: [embed],
          flags: MessageFlags.Ephemeral
        });
      ...
      state.serverSettings[guildId].allowedChannels = validChannels;
      await saveStateToFile();
      ...
      const embed = new EmbedBuilder()
        .setColor(0x00FF00)
        .setTitle('✅ Channels Updated')
        .setDescription(`Bot is now restricted to: ${channelMentions}${invalidChannels.length > 0 ? `\n\n⚠️ Invalid IDs: ${invalidChannels.join(', ')}` : ''}`);
      ...
      const embed = new EmbedBuilder()
        .setColor(0xFF0000)
        .setTitle('❌ Error')
        .setDescription('Failed to update channel settings.');
      await interaction.reply({
        embeds: [embed],
        flags: MessageFlags.Ephemeral
      });
...
async function showUserSettings(interaction, isUpdate = false) {
  try {
    ...
    // Only add the second button row if it fits (max 5 rows)
...
async function showChannelManagementModal(interaction) {
  if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
    const embed = new EmbedBuilder()
      .setColor(0xFF0000)
      .setTitle('🚫 Permission Denied')
  ...
  const placeholder = currentChannels.length > 0 
    ? currentChannels.map(id => `#${id}`).join(', ')
    : 'Enter channel IDs or type "all" for all channels';
  ...
  const modal = new ModalBuilder()
    .setCustomId('channel_management_modal')
...
async function handleTextMessage(message) {
  ...
  try {
    ...
    // Show simple thinking message without embed unless response format is embedded
    const userSettings = state.userSettings[userId] || {};
    const serverSettings = guildId ? (state.serverSettings[guildId] || {}) : {};
    const effectiveSettings = serverSettings.overrideUserSettings ? serverSettings : userSettings;
    const responseFormat = effectiveSettings.responseFormat || 'Normal';
    ...
    if (responseFormat === 'Embedded') {
      const embed = new EmbedBuilder()
        .setColor(0x5865F2)
        .setDescription('🤔 Gemini is thinking...');
