...
async function processAttachment(attachment, userId, interactionId) {
  ...
  if (shouldUploadToAPI) {
    ...
    try {
      ...
      if (contentType.startsWith('video/') || videoExtensions.includes(fileExtension) || contentType.startsWith('audio/') || audioExtensions.includes(fileExtension)) {
...
async function handleButtonInteraction(interaction) {
  ...
  const buttonHandlers = {
    ...
    'back_to_user_page1': showUserSettings,
...
async function processPromptAndMediaAttachments(prompt, message) {
  ...
  const guildId = message.guild?.id;
  const maxAttachments = guildId ? 5 : 10;

  if (attachments.length > 0) {
    const limitedAttachments = attachments.slice(0, maxAttachments);
    
    if (attachments.length > maxAttachments) {
      parts[0].text += `\n\n[Note: Only processing first ${maxAttachments} attachments out of ${attachments.length} total]`;
    }
...
client.login(token); embed responses')
        .setValue('Embedded')
        .setEmoji('📊')
        .setDefault(responseFormat === 'Embedded')
      );
...
async function showUserSettingsPage2(interaction, isUpdate = false) {
  try {
    ...
    const buttons3 = [
      new ButtonBuilder()
      .setCustomId('back_to_user_page1')
      .setLabel('Back')
      .setEmoji('◀️')
      .setStyle(ButtonStyle.Secondary)
    ];
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Additional personal bot preferences')
      .addFields({
        name: '🔄 Continuous Reply',
        value: `\`${continuousReply ? 'Enabled' : 'Disabled'}\``,
        inline: true
