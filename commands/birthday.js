...
async function showBirthdaySetup(interaction) {
  ...
  // Check how many birthdays the user has
  if (!state.birthdays) {
    state.birthdays = {};
  }
...
export async function handleBirthdayPrefSelect(interaction) {
  const [_, __, month, day, nameType] = interaction.customId.split('_');
  ...
  const forWhom = nameType === 'self' ? 'your' : 'their';
...
async function sendBirthdayWish(client, userId, data) {
  ...
  try {
    ...
    try {
        ...
        try {
          ...
          if (guild) {
            ...
            if (channel) {
              const mention = data.nameType === 'self' ? `<@${userId}>` : user.username;
              await channel.send({
                content: `🎉 It's ${mention}'s birthday today! 🎂`,
                embeds: [embed]
              });
            }
          }
      ...
      const fallbackMessage = `Happy Birthday, ${personName}! 🎂🎉 Wishing you an amazing day filled with joy!`;
      const embed = new EmbedBuilder()
        .setColor(0xFF69B4)
        .setTitle('🎉 Happy Birthday! 🎂')
      ...
      if ((data.preference === 'server' || data.preference === 'both') && data.guildId) {
        try {
          ...
          if (channel) {
            const mention = data.nameType === 'self' ? `<@${userId}>` : user.username;
            await channel.send({ 
              content: `🎉 It's ${mention}'s birthday!`, 
              embeds: [embed] 
            });
          }
        } catch (e) {}
