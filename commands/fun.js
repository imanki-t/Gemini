...
export const digestCommand = {
  name: 'digest',
  description: 'Get a weekly digest (7-day cooldown, max 100 messages)'
};
...
export async function handleDigestCommand(interaction) {
  ...
  const MAX_MESSAGES = 100; // Hard limit
  ...
  const messagesToAnalyze = allMessages.slice(-MAX_MESSAGES);
  ...
  try {
    ...
    try {
      ...
      summary = result.text || 'No clear topics identified.';
    } catch (error) {
...
Messages Analyzed: ${messagesToAnalyze.length} (limited to max 100 from ${allMessages.length} total)
...
${conversationText}`;
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '💬 Messages Analyzed', value: `${messagesToAnalyze.length} (from ${allMessages.length} total, max 100)`, inline: true },
      ...
      .setFooter({ text: `${interaction.guild.name} • AI-powered analysis` })
      .setTimestamp();
