...
function cleanHistoryFiles(history) {
 return history.map(entry => ({
   ...
   parts: entry.parts.map(part => {
     // If part has fileUri or fileData, replace with text description
     ...
     return part;
   }).filter(part => part.text) // Remove parts with no text
 })).filter(entry => entry.parts.length > 0); // Remove empty entries
}
...
async function handleModelResponse(initialBotMessage, modelName, systemInstruction, baseGenerationConfig, safetySettings, tools, history, parts, originalMessage, typingInterval, historyId, effectiveSettings, originalPrompt = '', allAttachments = []) {
 ...
 while (modelAttempts < maxModelAttempts && attempts > 0) {
   try {
     ...
     // ✅ Clean file references from history to prevent 403 errors after key rotation
     const cleanedHistory = cleanHistoryFiles(history);

     // Build request with proper config structure
     const request = {
       model: modelName,
       contents: [...cleanedHistory, { role: 'user', parts }], // ← Use cleanedHistory
       config: {
     ...
     if (isFilePermissionError && attempts > 0) {
       console.log(`🔄 [FIX] File permission error detected. Key rotation caused stale fileUri.`);
       
       // Re-process CURRENT message attachments with new key
       try {
         ...
         // IMPORTANT: Force re-fetch and clean history
         const freshHistory = await memorySystem.getOptimizedHistory(
           historyId,
           originalPrompt,
           modelName
         );
         history = cleanHistoryFiles(freshHistory);
         
         attempts = 3; // Reset attempts
         await delay(2000); // Give API time to settle
         continue;
       } catch (reProcessErr) {
         console.error("Failed to re-process attachments:", reProcessErr);
       }
     }
