...
await handleModelResponse(botMessage, modelName, finalInstructions, null, safetySettings, tools, history, parts, message, typingInterval, historyId, effectiveSettings, finalPromptForHistory, allAttachments);
}
...
while (modelAttempts < maxModelAttempts && attempts > 0) {
  try {
    ...
    if (isFilePermissionError && attempts > 0) {
      ...
      // Show typing indicator during key rotation
      originalMessage.channel.sendTyping().catch(() => {});
      const rotationTypingInterval = setInterval(() => {
        originalMessage.channel.sendTyping().catch(() => {});
      }, 4000);
      ...
      try {
        ...
        attempts = 3; 
        await delay(2000);
        clearInterval(rotationTypingInterval);
        continue;
      } catch (reProcessErr) {
        clearInterval(rotationTypingInterval);
    ...
    if (isRateLimitError) {
      ...
      // Show typing indicator during model fallback
      originalMessage.channel.sendTyping().catch(() => {});
