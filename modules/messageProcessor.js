...
// FIX: Guard against double-starts with atomic flag check
...
while (userQueueData.queue.length > 0) {
  ...
  try {
    ...
    // FIX: Ensure typing indicator stops even on error
...
// FIX: Start typing with centralized manager
...
try {
  ...
  if (message.poll || message.type === 46) {
    // FIX: Stop typing before returning
  ...
  if (!hasAnyContent) {
    // FIX: Stop typing before replying
    typingManager.stop(channelId);
  ...
  await handleModelResponse(botMessage, modelName, finalInstructions, null, safetySettings, tools, history, parts, message, channelId, historyId, effectiveSettings, finalPromptForHistory, allAttachments);
  
} catch (error) {
  console.error('Unhandled error in handleTextMessage:', error);
  // FIX: Ensure typing stops on any error
  typingManager.stop(channelId);
...
async function handleModelResponse(initialBotMessage, modelName, systemInstruction, baseGenerationConfig, safetySettings, tools, history, parts, originalMessage, channelId, historyId, effectiveSettings, originalPrompt = '', allAttachments = []) {
...
// FIX: Cleanup function to ensure typing stops and timeouts are cleared
const cleanup = () => {
  typingManager.stop(channelId);
  if (updateTimeout) {
    clearTimeout(updateTimeout);
...
try {
  while (modelAttempts < maxModelAttempts && attempts > 0) {
    try {
      ...
      // FIX: Stop typing once streaming begins
      typingManager.stop(channelId);
      ...
      if (isFilePermissionError && attempts > 0) {
        ...
        // FIX: Use centralized typing manager during rotation recovery
      ...
      if (isRateLimitError) {
        ...
        // FIX: Use centralized typing manager during model fallback
        typingManager.start(originalMessage.channel);
