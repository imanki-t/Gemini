...
client.on('messageCreate', async (message) => {
  try {
    ...
    if (guildId) {
      ...
      // Check if channel restrictions are enabled
      ...
      if (allowedChannels && allowedChannels.length > 0 && !allowedChannels.includes(channelId)) {
        return; // Bot ignores messages in non-allowed channels
      }
    }
    ...
    // Fixed: Continuous reply should respond to ALL messages in the channel
    const shouldRespond = (
      workInDMs && isDM ||
...
async function showChannelManagementMenu(interaction) {
  if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
    const embed = new EmbedBuilder()
      .setColor(0xFF0000)
      .setTitle('🚫 Permission Denied')
  ...
  const guildId = interaction.guild.id;
  const currentChannels = state.serverSettings[guildId]?.allowedChannels || [];
  ...
  const options = [
    new StringSelectMenuOptionBuilder()
      ...
      .setDescription('Allow bot in all channels')
      .setValue('all_channels')
      .setEmoji('🌐')
      .setDefault(currentChannels.length === 0)
  ];
  ...
  channels.forEach(channel => {
    options.push(
      new StringSelectMenuOptionBuilder()
        ...
        .setValue(channel.id)
        .setDefault(currentChannels.includes(channel.id))
    );
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription('Select the channels where the bot can respond.\n\n**Current:** ' + 
      (currentChannels.length > 0 
        ? currentChannels.map(id => `<#${id}>`).join(', ')
        : 'All channels'))
    .setFooter({ text: 'Select "All Channels" to allow bot in all channels' });
...
async function handleTextMessage(message) {
  ...
  try {
    ...
    // FIXED: Always show "Gemini is thinking..." as plain text, never as embed
    const userSettings = state.userSettings[userId] || {};
    const serverSettings = guildId ? (state.serverSettings[guildId] || {}) : {};
    const effectiveSettings = serverSettings.overrideUserSettings ? serverSettings : userSettings;
    const continuousReply = effectiveSettings.continuousReply || false;
    ...
    // FIXED: No mention in continuous reply mode
    const shouldMention = !continuousReply && !channelContinuousReply;
  const mention = shouldMention ? `<@${originalMessage.author.id}> ` : '';
  ...
  const updateMessage = () => {
    ...
    if (tempResponse.trim() === "") {
      botMessage.edit({
        content: `${mention}💭 Thinking...`,
        embeds: []
      }).catch(() => {});
    } else if (responseFormat === 'Embedded') {
      updateEmbed(botMessage, tempResponse, originalMessage, groundingMetadata, urlContextMetadata, effectiveSettings, mention);
    } else {
      botMessage.edit({
  ...
  while (attempts > 0 && !stopGeneration) {
    try {
      ...
      if (!isLargeResponse && responseFormat === 'Embedded') {
        updateEmbed(botMessage, finalResponse, originalMessage, groundingMetadata, urlContextMetadata, effectiveSettings, mention);
      }

      botMessage = await addSettingsButton(botMessage);
      if (isLargeResponse) {
        await sendAsTextFile(finalResponse, originalMessage, botMessage.id, !shouldMention);
      ...
      if (attempts === 0 || stopGeneration) {
        if (!stopGeneration) {
          ...
          const errorMsg = await originalMessage.channel.send({
            content: shouldMention ? `<@${originalMessage.author.id}>` : '',
            embeds: [embed]
          });
...
function updateEmbed(botMessage, finalResponse, message, groundingMetadata = null, urlContextMetadata = null, effectiveSettings, mention = '') {
  try {
...
client.login(token);
    const mention = shouldMention ? `<@${message.author.id}> ` : '';

    botMessage = await message.reply({
      content: `${mention}🤔 Gemini is thinking...`
    });
    ...
    for (const attachment of attachments) {
      ...
      if (fileTypes.includes(fileType)) {
        try {
          ...
          // If file is very large, skip inline inclusion (will be handled by processAttachment)
...
async function processAttachment(attachment, userId, interactionId) {
  ...
  // Handle media files (images, audio, video, PDF, GIF)
  ...
  // Handle text-based files
    ...
    try {
      ...
      if (fileContent.length > 1000000) {
        // For very large files, upload to Gemini Files API instead
        const sanitizedFileName = sanitizeFileName(attachment.name);
        const uniqueTempFilename = `${userId}-${interactionId}-${Date.now()}-${sanitizedFileName}`;
        const filePath = path.join(TEMP_DIR, uniqueTempFilename);
...
async function handleSelectMenuInteraction(interaction) {
  ...
  if (interaction.customId === 'user_model_select') {
    ...
    await saveStateToFile();
    await showServerSettings(interaction, true);
  } else if (interaction.customId === 'channel_select_menu') {
    // Handle channel selection/deselection
...
async function handleChannelSelection(interaction) {
  ...
  // If "all_channels" is selected, clear restrictions
  if (selectedChannels.includes('all_channels')) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Bot is now allowed in **all channels**.');
  ...
  const embed = new EmbedBuilder()
    .setColor(0x00FF00)
    .setTitle('✅ Channels Updated')
    .setDescription(`Bot is now restricted to: ${channelMentions}`);
...
async function showUserSettings(interaction, isUpdate = false) {
  try {
    ...
    // Check if server override is enabled
    if (guildId && serverSettings.overrideUserSettings) {
      const overrideEmbed = new EmbedBuilder()
        .setColor(0xFFAA00)
        .setTitle('⚠️ Server Override Active')
        .setDescription('This server has enabled settings override. Your personal settings will not take effect in this server, but you can still configure them for use in other servers or DMs.');

      try {
        await interaction.user.send({ embeds: [overrideEmbed] });
      } catch (error) {
        console.error('Failed to send DM about override:', error);
      }
    }
    ...
    if (guildId && serverSettings.overrideUserSettings) {
      embed.setDescription('⚠️ **Server Override Active** - Your settings won\'t apply in this server\n\nConfigure your personal bot preferences');
    }
